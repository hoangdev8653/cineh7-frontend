...
const MovieCard = ({ id, title, image, tags, rating }: MovieCardProps) => {
    const navigate = useNavigate();
    return (

        <div
            onClick={() => navigate(PATH.MOVIE_DETAIL.replace(':id', id.toString()))}
        ...
        >
            ...
            <div className="p-4">
                <div className="flex gap-2 mb-2">
                    {tags.map((tag, index) => (
                        <span key={index} className="text-[10px] uppercase font-bold tracking-wider text-slate-400 border border-slate-200 px-1.5 py-0.5 rounded">
                            {tag}
                        </span>
                    ))}
                </div>
