...
import { useNewsEventDetail } from '../../../hooks/useNewsEvent';
import { useMovies } from "../../../hooks/useMovie"

function NewsEventDetail() {
    const { id } = useParams();
    const { data: newsEvents } = useNewsEventDetail(id || "");
    const { data: movies } = useMovies();
    const tags = newsEvents?.content?.tags?.map((item: any) => item);
                ...
                <div className="container mx-auto px-4 md:px-16 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <nav className="flex items-center gap-2 text-xs md:text-sm font-bold tracking-tight">
                            ...
                            <span className="text-slate-300">/</span>
                            <span className="text-slate-400 truncate max-w-[150px] md:max-w-none">{newsEvents?.title}</span>
                        </nav>
                    </div>
                        ...
                        <div className="max-w-4xl">
                            ...
                            <div className="mb-12">
                                <p className="text-slate-700 leading-relaxed text-lg mb-8">
                                    ...
                                    <br />
                                    <span className="font-black text-slate-900">{newsEvents?.title}</span> chính là lựa chọn hoàn hảo giúp bạn tận hưởng trọn vẹn bộ phim yêu thích mà vẫn tiết kiệm chi phí.
                                </p>
                                ...
                                <ul className="space-y-4 mb-10 overflow-hidden">
                                    <li className="flex items-start gap-3 text-slate-700 font-medium">
                                        ...
                                        <span className="font-black">{newsEvents?.content?.summary}</span>
                                    </li>
                                </ul>

                                {newsEvents?.content?.sections?.map((section: any, index: number) => (
